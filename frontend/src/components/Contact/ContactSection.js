...
const Contact = () => {
  ...
  const API_BASE = import.meta.env?.VITE_API_BASE
    ? import.meta.env.VITE_API_BASE
    : (import.meta.env.DEV ? 'http://localhost:5000' : 'https://your‑production‑backend‑url.com');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/contact`, {
        method: 'POST',
        // Uncomment credentials if you need cookies/auth:
        // credentials: 'include',
        headers: { 'Content‑Type': 'application/json' },
        body: JSON.stringify(formData),
      });
      ...
      console.error('Fetch error:', err);
      toast.error(`Error connecting to server. Please check your backend. (${err.message})`);
    } finally {
      setLoading(false);
    }
  };
